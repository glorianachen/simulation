...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 24 11:46:49 2020
...
class Scheduler():
    ...
    def runSim(self, eventHandler, endTime):
        while len(self.FEL)>0:
            ...
            # print( nextEvent )
            print( 'current timestamp is %s ' % (timestampToStr(nextEvent.timestamp) ))
            print('current route is %s ' % (nextEvent.route))
            print( 'current bus for that route is %s ' % (nextEvent.bus.numOnRoad) )
            print('current passenger count is %s ' % (nextEvent.bus.peopleOnBus))
